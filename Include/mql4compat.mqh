...
#define Bars (::Bars(_Symbol, _Period))  // Add this macro
// int Digits=_Digits;
...
{   
   ...
   if(CopyOpen(symbol,timeframe, 0, Bars, Arr)>0) 
        return(Arr[_index]);
   else return(-1);
}
...
{
   // Only print when not in backtest
...
bool SafeOrderSelect(int ticket) {
   ...
   // In backtest, suppress spam by not printing
   if (IsTesting()) {
      return false;
   }
   // In live trading, still log the error
   Print("Order ", ticket, " not found. Error: ", GetLastError());
   return false;
}

// --- Utility function section ---
bool SafeHistoryOrderSelect(ulong ticket)
{
